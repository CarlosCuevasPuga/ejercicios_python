...
while True:
    try:
        ...
        if opcion == 0:
                ...
                try:
                    ...
                    if resultado_usuario == resultado:
                        ...
                        if racha_aciertos > racha_usuario:
                            racha_usuario = racha_aciertos
                        if racha_aciertos == 3 and not trofeo_bronce:
                            print("LOGRO DE BRONCE DESBLOQUEADO")
                            trofeo_bronce = True
                        if racha_aciertos == 7 and not trofeo_plata:
                            print("LOGRO DE PLATA DESBLOQUEADO")
                            trofeo_plata = True
                        if racha_aciertos == 10 and not trofeo_oro:
                            print("LOGRO DE ORO DESBLOQUEADO")
                            trofeo_oro = True
                        if racha_aciertos == 10 and vidas == 1 and not trofeo_superviviente:
                            print("LOGRO DE VALIENTE DESBLOQUEADO")
                            trofeo_superviviente = True
                        if trofeo_bronce and trofeo_plata and trofeo_oro and trofeo_superviviente:
            ...
            print(f"Logro de bronce: acierta tres operaciones consecutivas en una misma partida -> {"SI" if trofeo_bronce else "NO"} obtenido")
            print(f"Logro de plata: acierta siete operaciones consecutivas en una misma partida -> {"SI" if trofeo_plata else "NO"} obtenido")
            print(f"Logro de oro: acierta diez operaciones consecutivas en una misma partida -> {"SI" if trofeo_oro else "NO"} obtenido")
            print(f"Logro de valiente: resuelve 10 ecuaciones seguidas con 1 vida -> {"SI" if trofeo_superviviente else "NO"} obtenido")
            print(f"Logro Calculatron: consigue todos los trofeos -> {"SI" if trofeo_calculatron else "NO"} obtenido")